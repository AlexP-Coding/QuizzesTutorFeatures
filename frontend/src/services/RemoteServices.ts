...
export default class RemoteServices {
  ...
  static async updateWeeklyScores(dashboardId: number | undefined) {
    return httpClient
  ...
  static async deleteWeekly(weeklyId: number) {
    return httpClient
      .delete(`/students/weeklyscores/${weeklyId}`)
      .catch(async (error) => {
        throw Error(await this.errorMessage(error));
      });
  }
  ...
  static async deleteDifficultQuestions(difficultQuestionId: number) {
    return httpClient
  ...
  static async deleteFailedAnswers(failedAnswerId: number) {
    return httpClient
